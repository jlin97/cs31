...
{
	if (nTimesAttacked == 1) {
		return true;      // true means the robot is dead
	}
	else if (nTimesAttacked < 1) {
		if (m_arena->determineNewPosition(this->m_row, this->m_col, dir)) {
			nTimesAttacked++;
			return false;
		}
		else {
...
void Player::moveOrAttack(int dir)
{
	...
	switch (dir) {
	case UP:
		if (m_arena->nRobotsAt(this->m_row - 1, this->m_col) > 0) {
			m_arena->attackRobotAt(this->m_row - 1, this->m_col,dir);
		}
		else {
		...
		break;
	case LEFT:
		if (m_arena->nRobotsAt(this->m_row, this->m_col - 1) > 0) {
			m_arena->attackRobotAt(this->m_row, this->m_col - 1, dir);
		}
		else {
